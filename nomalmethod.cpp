                        ...
                        std::vector<std::vector<Matrix54i>> &c_route)
{
    ...
    for (Matrix54i &state : movable)
    {
        ...
        // board_simples time instrumentation.
        start = std::chrono::system_clock::now();
        ...
        // to_hashable time instrumentation.
        start = std::chrono::system_clock::now();
        std::string hash_puzzle = to_hashable(simple_puzzle);
        end = std::chrono::system_clock::now();
        double time2 = static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
        // printf("%.0lf\n", time);
        ...
        if (exist(hash_puzzle, puzzle_state))
        {
            ...
            if (clear(state))
            {
                ...
                double time3 = static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
                printf("%.0lf[ns]\n", time3);
